...
- **Structure advisor, precision editor, tightening advisor: a non-Claude family; DeepSeek V4 Pro today.** The cost of in-family editing is on the record: run 3 seated all four prose seats on one model and the in-family editor caught a sixth of what the cross-family editor caught, and echoed the writer's own error. Grok 4.5 is licensed as an editing family (ruled 2026-09-12, knowing it reviews the origin repository's pull requests); which non-Claude family takes the editing seats is decided by the editor bake-off staged the same day (candidates DeepSeek as baseline, Kimi K3, GPT-5.6 Sol, Grok 4.5, Qwen 3.7 Max, Mistral Large, Gemini 3.1 Pro as control; scored against run 3's natural fault ledger). A Claude model in any prose seat is out.
- **Interviewer (Lamb) and assessor: non-Claude, DeepSeek V4 Pro by default.** The interviewer is a prose seat and follows the writer's rule; the assessor reads the reader, not the prose, and the reader ruled it non-Claude regardless ("super important that we get this right"). A clearer candidate from the bake-off may take the assessor's seat.
