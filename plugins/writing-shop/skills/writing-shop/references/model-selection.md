...
- **Precision editor: `openai/gpt-5.6-sol`; runner-up `x-ai/grok-4.5`. Tightening advisor: `google/gemini-3.1-pro-preview` on the numbers, or `x-ai/grok-4.5` if the reader extends Gemini's writer-seat rejection of 2026-08-31 to every seat; runner-up Grok either way. Structure advisor: a non-Claude family other than the writer's, Grok by default.** Measured on 2026-09-12 by the editor bake-off: run 3's DeepSeek draft and its twelve-item natural fault ledger, one letter per seat per family at temperature 0.8, scored by Claude agents against the ledger (the full report is in the session's scratchpad; the numbers are here). Fowler recall, caught plus half of partial over twelve: Sol 0.625 with one false alarm and no harmful fix; Grok 0.292 with neither; Qwen 0.208 with two false alarms; Gemini 0.167 with three harmful fixes; DeepSeek editing its own family's draft 0.125 with three false alarms, two harmful fixes and five gate findings; Mistral 0.083 with seven false alarms and three harmful fixes (the scorer: "do not seat"); Kimi K3 produced no visible letter, its whole 20,000-token ceiling going to hidden reasoning. Orwell: Gemini eight good cuts and one bad of eleven with four signposts caught, but the bad cut removed a definition; Grok three good and two bad of eight, the only tightener to reach a costumed reversal frame, and the most in character; the others two to four good against two to three bad. The seats should differ, because each leader failed plainly in the other chair; a relay confined to one family runs Grok, second in both. The in-family cost stayed on the record: DeepSeek editing DeepSeek was fifth of six. One sample per family; a difference of one catch is noise, so read shapes. Grok 4.5 is licensed as an editing family knowing it reviews the origin repository's pull requests. A Claude model in any prose seat is out.
- **Interviewer (Lamb): non-Claude, DeepSeek V4 Pro by default; a prose seat under the writer's rule. Assessor: non-Claude always** ("super important that we get this right"); DeepSeek V4 Pro holds it until a validation run on real reader answers, and the bake-off nominates `openai/gpt-5.6-sol` for it (eleven of twelve ledger items touched, one false alarm, none harmful: careful reading against a reference is the habit the seat needs), with Grok as the alternative if false alarms weigh more than recall.
