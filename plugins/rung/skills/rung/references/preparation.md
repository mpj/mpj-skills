...
The learner ledger and reader transcript receive no speculative turns. Continue the writer from a private copy of its existing history, with the same anchors and stable prefix; it supplies the conversational part and its closing question together. Kept work also continues the interviewer privately after editing. These are provisional continuations of the same seats, not newly anchored seats. Reuse their per-seat routing keys where supported. Only the candidate's storage receives those calls. The live histories remain available for a different reply without having to undo imagined evidence.
...
For conversation, prepare the writer's next part, closing question and concretes in one call, run the usual factual, leak and lint checks, and collect blocking findings into at most one speculative repair request. A small length overrun allowed by conversational lint is recorded, not repaired solely for length. Check the assembled delivery, including whether the question fits. Kept work leaves the question slot for its separate interviewer after editorial passes and uses strict lint. Unfinished passes or unresolved blocking findings stay explicit and block release. There is no speculative repair loop.
...
If preparation is incomplete when the reply arrives, reuse any completed draft that fits the actual result and finish only the missing work. A prepared writer draft can still save a call even when checks are pending, or when kept work still owes its interviewer question.
