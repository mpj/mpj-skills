...
* **Only material that will be identical next session goes in the system message.** The anchors, the standing prompt and the prohibitions qualify; the ground-truth sheet does not, because it is a fact about this session's subject, and neither does anything else the relay assembles per run. Providers cache on an exact prefix match, so one per-run sentence at the front costs the whole prefix on every call of every seat. `model-selection.md` told the writer's seat to put the sheet in the system message until 2026-09-13, which is where this rule comes from.
* **Log what the cache did, and know the window is shorter than the relay.** A router reports the cache in the usage object (OpenRouter puts it in `usage.prompt_tokens_details`, as `cached_tokens` and, where the provider prices writes, `cache_write_tokens`); no flag turns this on and the parameter that used to is deprecated and does nothing, checked 2026-09-13. Two clocks run against a serialized relay. The provider's own cache expires on its own schedule, which for one family is five minutes unless the call asks for an hour. The router's sticky session to a given provider expires after ten minutes of inactivity, which loses the prefix a different way, by sending the next call somewhere the prefix never was. Seven seats taking turns can put more than ten minutes between one seat's calls, so a run that cares should log the numbers rather than assume them.
